...
resource "aws_iam_role_policy" "recovery" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      Condition = { "ForAllValues:StringLike" = { "dynamodb:LeadingKeys" = ["V1#*#*"] }, "ForAnyValue:StringEquals" = { "dynamodb:EnclosingOperation" = ["TransactWriteItems"] } } },
      { Sid = "UpdateRecoveryCheckpoint", Effect = "Allow", Action = ["dynamodb:UpdateItem"], Resource = local.authority_arn,
      Condition = { "ForAllValues:StringEquals" = { "dynamodb:LeadingKeys" = ["V1#CHECKPOINT"] }, "ForAnyValue:StringEquals" = { "dynamodb:EnclosingOperation" = ["TransactWriteItems"] } } },
