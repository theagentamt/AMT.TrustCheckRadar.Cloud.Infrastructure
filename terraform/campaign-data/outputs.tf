...
output "downstream_contract" {
  ...
  value = {
    ...
    feature_queue_url       = local.enabled ? aws_sqs_queue.feature[0].id : null
    feature_queue_arn       = local.enabled ? aws_sqs_queue.feature[0].arn : null
    feature_dlq_arn         = local.enabled ? aws_sqs_queue.feature_dlq[0].arn : null
    ...
    model_repository_arn    = local.enabled ? aws_ecr_repository.model[0].arn : null
    model_repository_url    = local.enabled ? aws_ecr_repository.model[0].repository_url : null
