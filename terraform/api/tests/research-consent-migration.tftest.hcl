...
run "coordinated_candidate_is_pinned_and_consent_stays_closed" {
  ...
  assert {
    condition     = length(aws_iam_role_policy.research_migration_boundary) == 5 && alltrue([for k, doc in data.aws_iam_policy_document.research_migration_boundary : anytrue([for st in doc.statement : st.sid == "DenyProviderCredentialsAndDispatch" && st.effect == "Deny" && contains(st.actions, "secretsmanager:GetSecretValue")])]) && alltrue([for k in ["analysis", "snapshot", "web_risk", "purchase"] : anytrue([for st in data.aws_iam_policy_document.research_migration_boundary[k].statement : st.sid == "DenyLegacySettlementAndWrites" && st.effect == "Deny" && contains(st.actions, "dynamodb:PutItem") && contains(st.actions, "dynamodb:UpdateItem")])]) && anytrue([for st in data.aws_iam_policy_document.research_migration_boundary["web_risk"].statement : st.sid == "DenyRetiredDirectLookupDataAccess" && st.effect == "Deny"])
