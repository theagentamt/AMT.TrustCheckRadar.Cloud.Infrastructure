...
variable "enabled" {
  description = "Provision only an inactive isolated Dev candidate; this does not publish routes or activate provider work."
  type        = bool
  default     = false
  validation {
  ...
  validation {
    condition = var.deployment == null ? true : try(
      ...
      alltrue([for name, a in var.deployment.artifacts :
        ...
        can(regex("^releases/[a-f0-9]{40}/${ { consumer = "recovery_consumer", evaluator = "recovery_evaluator" }[name]}\\.zip$", a.key)) &&
    ...
    error_message = "Use exact environment-scoped dependencies and recovery consumer/evaluator packages from one immutable release."
  }
}
