...
Research can stay paused during campaign account deletion. Lambda bridge has no global research kill-switch dependency. At the assessed base, Terraform coupled its event-source mapping to `local.active` (which is false for the paused candidate), and hardcodes recovery schedule/environment disabled. The infrastructure deletion-only activation overlay now addresses that configuration gap, while its Dev selection remains null. Keep global campaign kill switch true, publisher/cluster admission false and queue/observation mappings disabled, lifecycle false and its three jobs disabled. Enable only the deletion mapping/recovery schedule and bridge flags after qualification.
...
At the assessed f922 base, account_data.validate_config permitted ACCOUNT_DELETION_ENABLED/finalizer=true with CAMPAIGN_RECOVERY_WRITES_ENABLED=false. Root's activation overlay sets true, but source should fail closed to prevent accepting a command without its required durable recovery job. Lambda PR61 source997e265f7edf10ce7369481f7d576b4276e8c418 now rejects that configuration and covers the handler paths; no durable schema/IAM change. All other conclusions above are qualification/infra dependencies, not missing receipt implementations.
