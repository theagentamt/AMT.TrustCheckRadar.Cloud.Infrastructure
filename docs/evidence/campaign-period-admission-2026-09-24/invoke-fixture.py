...
require(isinstance(cases,tuple) and len(cases)<=50 and len(set(cases))==len(cases))
...
r={'schemaVersion':1,'observedAtUtc':datetime.now(timezone.utc).isoformat(),'sourceSha':j['sourceSha'],'zipSha256':j['zipSha256'],'runId':j['runId'],'runtime':c['Runtime'],'architectures':c['Architectures'],'runtimeVersionArn':c.get('RuntimeVersionConfig',{}).get('RuntimeVersionArn'),'cases':[],'allPassed':False,'syntheticOnly':True,'historicalCoverageApproved':False,'productionActivation':False,'limits':['Synthetic snapshot reconstruction, not native AWS backup restoration.','Fixture seeding role is not production completion IAM acceptance.','Some races/lost acknowledgments are deliberately injected around actual AWS SDK operations.','Period producer cases inject SQS and candidate-index discovery; logical outbox records share the isolated users fixture table. These do not qualify actual transport or separate outbox-table bindings.']}
