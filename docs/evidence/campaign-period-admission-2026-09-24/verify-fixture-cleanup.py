...
j=json.loads(Path('/tmp/amt-period-admission-fixture-journal.json').read_text());s=boto3.Session(profile_name='trustcheckradar',region_name='us-east-1');cfg=Config(connect_timeout=5,read_timeout=15,retries={'total_max_attempts':1})
...
Path('/tmp/amt-period-admission-fixture-cleanup-readback.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
