...
1. The owner specified `support@andmorethings.com` for all alerts. A follow-up local Terraform apply added only the Dev email subscription (one addition, no changes or deletions); AWS readback verified the endpoint and `PendingConfirmation=true`. Subscription ID: `12a4aaad-33da-4afa-af2c-7698bfe9a8ef`. After the owner confirmed the email, AWS readback returned `PendingConfirmation=false`. SNS accepted one labeled test alert (`TEST - AMT Dev URL resolver alerts`), message ID `bb01cd67-9c9e-566d-82aa-ae4e19158b8a`. The owner subsequently confirmed receipt of this test email in the task. This verifies the SNS-to-email delivery path; a real CloudWatch alarm transition was not exercised. A post-confirmation full resolver plan reported no changes. UAT/Prod tfvars record the same recipient but remain disabled and were not deployed. The SNS email-delivery acceptance check is complete.
